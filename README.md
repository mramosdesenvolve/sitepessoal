...
- **Zoom inicial do grafo (`fitGraph` em `RhizomeGraph.tsx`)**: `onEngineStop` pode disparar antes de a força de colisão (aplicada assincronamente em `handleFgRef`) convergir de verdade, deixando um `zoomToFit` prematuro e nós sobrepostos. Por isso há uma segunda chamada de segurança ~2.2s após o mount, além das dos primeiros `onEngineStop`. Um `userInteractedRef` (setado via `onZoom`/`onNodeDrag`) impede que essa rede de segurança sobrescreva zoom/pan manual do usuário — `programmaticZoomUntilRef` (a janela em que se ignora o próprio `onZoom` do `zoomToFit`) começa em `Infinity`, não `0`: a lib dispara um `onZoom` no mount, antes do primeiro `fitGraph()` rodar, e com `0` esse evento já marcava `userInteractedRef` como `true` permanentemente, cancelando todo reenquadramento futuro silenciosamente.
