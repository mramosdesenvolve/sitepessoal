...
- **Zoom inicial do grafo (`fitGraph` em `RhizomeGraph.tsx`)**: `onEngineStop` pode disparar antes de a força de colisão (aplicada assincronamente em `handleFgRef`) convergir de verdade, deixando um `zoomToFit` prematuro e nós sobrepostos. Por isso há uma segunda chamada de segurança ~2.2s após o mount, além das dos primeiros `onEngineStop`. Um `userInteractedRef` (setado via `onZoom`/`onNodeDrag`) impede que essa rede de segurança sobrescreva zoom/pan manual do usuário.
